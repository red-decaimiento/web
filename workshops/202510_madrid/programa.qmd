...
gt::gt(d28) |> 
  gt::fmt_markdown(column = c(Coordina, Material)) |>
  tab_style(
...
d29 <- d |> 
  filter(day == 29) |> 
  dplyr::select(-jornada, -day, Actividad = Mesa, Coordina, -slides) |> 
  mutate(Hora = str_remove_all(Hora, "NA - NA"))

gt::gt(d29) |> 
  gt::fmt_markdown(column = Coordina) |>
  tab_style(
    style = cell_fill(color = "#dfece9"),
    locations = cells_body(
      rows = type == "break")
...
d30 <- d |> 
  filter(day == 30) |> 
  rename(Material = slides) |> 
  dplyr::select(-jornada, -day)

gt::gt(d30) |> 
  fmt_markdown(colum = c(Coordina, Material)) |>
  tab_style(
    style = cell_fill(color = "#dfece9"),
    locations = cells_body(
      rows = type == "break")
